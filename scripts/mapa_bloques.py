...
def main():
    ...
    # ⚠️ La clave NO puede ser sólo el nombre. El XD repite nombres genéricos —«Image Place
    # Holder», «Rectángulo»— en nodos distintos, y con `setdefault` por nombre el primero le
    # imponía su fill a todos los demás. En el Tema 2 eso pintaba de #FEDDB4 un bloque que el PDF
    # tiene en #F7E3E6, y el comparador lo denunciaba como ancho sin pareja. Se indexa por
    # `nombre|ANCHOxALTO`, que sí distingue, y se deja el nombre solo como respaldo.
    estilo = {}
    for extra in (['--con-tamano'], []):
        for ln in corre('xd_read.py', extra).splitlines():
            m = re.match(r'\[(\w+)\].*?·\s*(.+)$', ln)
            if m:
                val = (re.sub(r'^\[\w+\].*?(rect|path|circle|ellipse|line)\b', r'\1', ln)
                       .split('·')[0].strip())
                nom_nodo = m.group(2).strip()
                mt = re.search(r'\s(\d+)x(\d+)\s', ln)
                if mt:
                    estilo.setdefault(f'{nom_nodo}|{mt.group(1)}x{mt.group(2)}', val)
                estilo.setdefault(nom_nodo, val)
    ...
    for y, x, w, h, nom in filas:
        ...
        if len(hermanos) >= 3:
            colores = {estilo.get(f'{n}|{w}x{h}', estilo.get(n, '')) for n in hermanos}
            if len({c for c in colores if '#' in c}) > 1:
                marcas.append('[HOVER?]')
        est = estilo.get(f'{nom}|{w}x{h}', estilo.get(nom, ''))
        print(f'  ({x:>5},{y:>6}) {w:>5}x{h:<5} {est:<58} {nom[:28]:<28} '
              f'{" ".join(marcas)}')
