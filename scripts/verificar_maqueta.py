...
# Todas las rutas de aqui abajo son relativas a la RAIZ del entregable (`src/views/...`).
# Sin esto la herramienta solo funciona si se la invoca desde esa raiz, y desde `scripts/`
# muere en el primer `open('src/config/global.js')` -- mismo fallo de clase que el puerto
# escrito a fuego: no dice de donde viene.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import config as C  # noqa: E402

os.chdir(C.ENTREGABLE)
...
#
# ⚠️ Este apartado medía lo que no era. En CF01 dijo «5 imágenes distintas» y dio la pantalla por
# buena: los cinco archivos eran distintos entre sí, pero las 20 preguntas los repartían de cuatro
# en cuatro, y el kit necesita **una por pregunta mostrada**. Manuel entregó diez. La duplicación
# no estaba en los ficheros sino en las REFERENCIAS, que es lo que no se estaba contando.
paso('1g. imágenes de la actividad: ficheros, referencias y cobertura')
...
if dupes:
    print('  ficheros byte a byte iguales: ' + ' | '.join(dupes))
    fallos.append(f'{len(dupes)} grupos de imágenes de actividad repetidas')

act = 'src/views/Actividad.vue'
if os.path.exists(act):
    src_act = open(act).read()
    refs = re.findall(r"imagen:\s*'([^']+)'", src_act)
    n_preg = len(re.findall(r'\btexto:', src_act)) and len(re.findall(r'^\s{8,10}\{', src_act, re.M))
    n_preg = len(refs)                      # una `imagen:` por pregunta
    distintas = len(set(refs))
    muestra = re.search(r'MOSTRAR_PREGUNTAS\s*=\s*(\d+)', src_act)
    esperadas = int(muestra.group(1)) if muestra else 10    # el kit muestra 10 de las 20
    print(f'  {len(firmas)} ficheros · {n_preg} preguntas · {distintas} imágenes distintas '
          f'referenciadas (el kit muestra {esperadas} preguntas)')
    if distintas < esperadas:
        print(f'     ❌ con {distintas} distintas, dos preguntas de la misma tanda salen con la '
              'misma foto. Hacen falta al menos una por pregunta mostrada.')
        fallos.append(f'la actividad sólo referencia {distintas} imágenes distintas '
                      f'({esperadas} preguntas mostradas)')
    huerfanas = sorted({os.path.basename(f) for f in glob.glob('src/assets/actividad/imagen*.png')}
                       - {r.split('/')[-1] for r in refs})
    if huerfanas:
        print(f'     ⚠️  en assets pero sin usar: {", ".join(huerfanas)}')

# ---------------------------------------------------------------- 1g2. tarjetas sin `h-100`
# Cuatro `.tarjeta--boton` en una fila tienen alturas distintas si su texto no mide lo mismo, y el
# diseño las quiere iguales. Manuel añadió `.h-100` a las cuatro del Tema 3 de CF01; en el
# entregable que él cerró (CF1_63720048) están las 12 con `h-100`, sin una sola excepción.
# Sólo `.tarjeta--boton`: el `.cajon` va a ancho completo y allí NINGUNO lleva `h-100` (11 de 11),
# así que meterlo aquí daría falsos positivos.
paso('1g2. tarjetas de fila sin `h-100`')
sin_h100 = []
for v in sorted(glob.glob('src/views/*.vue')):
    for ln, linea in enumerate(open(v).read().splitlines(), 1):
        m = re.match(r'\s*\.(tarjeta--boton)([\w.-]*)$', linea.strip())
        if m and 'h-100' not in m.group(2):
            sin_h100.append(f'{os.path.basename(v)}:{ln}  .{m.group(1)}{m.group(2)}')
print('  ' + ('\n  '.join(sin_h100) if sin_h100 else 'ninguna'))
if sin_h100:
    fallos.append(f'{len(sin_h100)} tarjetas de fila sin h-100')
...
try:
    ...
    proc = subprocess.Popen(['google-chrome', '--headless=new', '--disable-gpu', '--no-sandbox',
                             ...
                             '--window-size=900,1200', f'{BASE}?noaos#/'],
    ...
    # AOS deja los bloques con un `transform: translate3d(...)` hasta que entran en el
    # viewport, y a 485x800 la mayoria no entra nunca. Midiendo asi, el `right` es el del
    # bloque DESPLAZADO y salen desbordes que no existen (28 falsos positivos en CF01).
    # Chrome headless no abre ventanas de menos de 500 px: sin forzar las metricas del
    # dispositivo, un movil de 360 se mide en realidad a 500 y no se prueba nunca.
    i[0] += 1
    ws.send(json.dumps({'id': i[0], 'method': 'Emulation.setDeviceMetricsOverride',
                        'params': {'width': ANCHO_MOVIL, 'height': 1200,
                                   'deviceScaleFactor': 1, 'mobile': True}}))
    while json.loads(ws.recv()).get('id') != i[0]:
        pass
    time.sleep(2)
    MEDIDA_MOVIL = '''(()=>{const w=document.documentElement.clientWidth;const o=[];
    document.querySelectorAll('*').forEach(e=>{const r=e.getBoundingClientRect();const cs=getComputedStyle(e);
      if(r.right>w+2 && cs.overflowX==='visible' && cs.position!=='fixed'
         && !e.closest('.slyder-f__main,.scroll-horizontal,.tabla-a,.creditos,.horizontal-scroll__wrapper,.accesibilidad,.barra-avance'))
        o.push(e.tagName+'.'+(e.className||'').toString().slice(0,44));});
    return JSON.stringify({w, n:o.length, ej:o.slice(0,5)})})()'''
    total_movil, peor = 0, []
    for ruta in RUTAS[:11]:
        ev(f'location.hash = {json.dumps("#/" + ruta)}')
        time.sleep(2.2)
        ev("""document.querySelectorAll('[data-aos]').forEach(e=>{e.classList.add('aos-animate');
             e.style.transform='none';e.style.opacity=1})""")
        time.sleep(0.6)
        d = json.loads(ev(MEDIDA_MOVIL))
        total_movil += d['n']
        if d['n']:
            peor.append((ruta or 'portada', d['n'], d['ej'][:2]))
    print(f'  clientWidth={d["w"]}  rutas revisadas: {len(RUTAS[:11])}  '
          f'elementos que se salen: {total_movil}')
    for ruta, n, ej in peor:
        print(f'    {ruta}: {n}   {ej}')
    if total_movil:
        fallos.append(f'{total_movil} elementos se desbordan en móvil')
    ws.close()
    proc.kill()
    ...
    for r in RUTAS:
        ...
        for _ in range(20):                              # la vista monta y las imágenes cargan
            ...
            if ev("[...document.images].every(i=>i.complete)"):
                break
        # `?noaos` NO quita el `transform` inicial de AOS: los bloques que no han entrado en
        # el viewport siguen desplazados y su `bottom` mide de mas. Hay que neutralizarlo a
        # mano o salen cajas «fuera de su columna» que estan en su sitio.
        ev("""document.querySelectorAll('[data-aos]').forEach(e=>{e.classList.add('aos-animate');
             e.style.transform='none';e.style.opacity=1})""")
        time.sleep(0.8)
